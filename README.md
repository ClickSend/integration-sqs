...
-   GitHub CLI installed on your PC -  [GitHub-CLI](https://cli.github.com/manual/installation)

-   AWS CLI installed on your PC - [AWS-CLI](https://aws.amazon.com/cli/) 

-   NodeJs installed on your PC - [NodeJS](https://nodejs.org/en/)
...
2.  Install [GitHub-CLI](https://cli.github.com/manual/installation) using your terminal or cmd. 
...
4.  Browse to the producer nodejs folder from your command line / shell.
...
6.  There are two ways to obtain the credentials and permissions depending on whether you are working under an organisation or are the AWS administrator.
...
7. Configure your AWS CLI by setting up the .AWS folder directory and creating credential (obtained from above) and config (your region) files under this directory. Make sure these files do not have any extensions. Follow these links for more instructions:
...
10. Select a name for the queue that’s relevant to you. There's no need to change any settings/configuration/access policies; the default settings are enough.
...
12. After creating the queue, a window with the following message should appear on your screen. Copy the queue URL.
...
13. #### This guide shows two ways send SMS to AWS SQS queue; using Java or NodeJS.

    #### Option 1: Using Java

    Open the producer java folder as a Maven project from any IDE. For example, on
    visual studio code:
...
14. Change the from, to, and message content. Check the comments in the file for more details:
...
16. To check whether this message has reached the queue, click the
...
17. Then click “Poll for messages”, there should be messages available.
...
Now that our Lambda function is created, we need to add code this function that enables
it to send the messages via the ClickSend API.

22. Browse to the consumer/Lambda folder on your computer from terminal/cmd.
...
28. Get your ClickSend credential, follow this link:
    [ClickSend Credentials](https://dashboard.clicksend.com/account/subaccounts?utm_source=github&utm_medium=referral&utm_campaign=integration-sqs&utm_content=cs-credentials)

29. Set your ClickSend credentials on AWS Lambda, we need to create two environment variables on your Lambda function. Go to the configuration tab and click on "Environment variables". Then click on "Edit".
